...
export default function TaxInformationPopOut() {
  ...
  // Function to add tax deadline to calendar
  const addTaxDeadlineToCalendar = () => {
    const currentYear = new Date().getFullYear();
    const deadlineText = STATE_TAX_DATA[selectedState].filingDeadline;
    
    // Extract month and day from the deadline text (typically "April 15")
    const monthMap: {[key: string]: number} = {
      "January": 0, "February": 1, "March": 2, "April": 3, "May": 4, "June": 5,
      "July": 6, "August": 7, "September": 8, "October": 9, "November": 10, "December": 11
    };
    
    // Parse the deadline text to extract month and day
    const match = deadlineText.match(/(\w+)\s+(\d+)/);
    if (match) {
      const month = monthMap[match[1]];
      const day = parseInt(match[2]);
      
      if (!isNaN(month) && !isNaN(day)) {
        // Create deadline date (use next year if the current date is past this year's deadline)
        const now = new Date();
        let deadlineYear = currentYear;
        const deadlineDate = new Date(deadlineYear, month, day);
        
        if (now > deadlineDate) {
          deadlineYear = currentYear + 1;
          deadlineDate.setFullYear(deadlineYear);
        }
        
        // Create calendar event
        const newEvent = {
          id: `tax-deadline-${selectedState}-${deadlineYear}`,
          title: `${STATE_TAX_DATA[selectedState].name} Tax Filing Deadline`,
          category: "finance",
          date: deadlineDate,
          description: `Tax filing deadline for ${STATE_TAX_DATA[selectedState].name} state taxes.`
        };
        ...
        const existingEventIndex = events.findIndex((e: any) => e.id === newEvent.id);
        ...
        // Show confirmation
        alert(`Added tax deadline for ${STATE_TAX_DATA[selectedState].name} to calendar: ${deadlineText}, ${deadlineYear}`);
      }
    }
  };
  ...
  // Calendar integration functionality is implemented above
