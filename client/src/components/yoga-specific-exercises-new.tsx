...
// Component for a single yoga exercise card with its own closed/open state
function YogaExerciseCard({
  exercise,
  loadExerciseVideo,
  loadingVideos,
  exerciseVideos
  ...
  const handleClose = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    console.log("YogaExerciseCard close button clicked");
    setIsOpen(false);
  };
...
export default function YogaSpecificExercises({
  ...
  // Map the category prop to the actual category used in YOGA_EXERCISE_SETS
  const getCategoryMapping = (): keyof typeof YOGA_EXERCISE_SETS => {
    switch (category) {
      case 'beginner': return 'beginnerPoses';
      case 'intermediate': return 'standingPoses';
      case 'advanced': return 'balancePoses';
      default: return 'beginnerPoses';
    }
  }
  ...
  return (
    <div className={`p-6 rounded-lg ${backgroundColor}`}>
      ...
      <div className="space-y-4">
        {YOGA_EXERCISE_SETS[selectedCategory].slice(0, maxExercises).map((exercise) => (
          <YogaExerciseCard
            key={exercise.id}
            exercise={exercise}
